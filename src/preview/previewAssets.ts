...
export function getPreviewAssetUris(webview: vscode.Webview, context: vscode.ExtensionContext): PreviewAssetUris {
  ...
  const katexStyleUri = webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, 'media', 'katex.min.css'));
  return { styleUri, scriptUri, hljsStyleUri, katexStyleUri };
}
