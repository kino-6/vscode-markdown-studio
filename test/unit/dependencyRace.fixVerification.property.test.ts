...
// ---------------------------------------------------------------------------
// Task 5: Fix Verification — Property-Based Tests (5.1 & 5.2)
// ---------------------------------------------------------------------------
...
/**
 * Generates random DependencyStatus objects where javaPath is always
 * undefined (simulating Java/Corretto not installed). Other fields vary.
 */
const statusWithoutJavaArb: fc.Arbitrary<DependencyStatus> = fc
  .record({
    allReady: fc.constant(false),
    browserPath: fc.option(
      fc.stringMatching(/^\/[a-z]{1,8}(\/[a-z]{1,8}){1,3}$/),
      { nil: undefined }
    ),
    errors: fc.array(
  ...
  .map(({ allReady, browserPath, errors }) => ({
    allReady,
    javaPath: undefined,
    browserPath,
    errors,
...
describe("Fix Verification: Dependency Gate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  /**
   * 5.1 Property: For any DependencyStatus where browserPath is undefined,
   * the chromium gate SHALL return false and the error message SHALL contain
   * "Setup Dependencies".
   *
   * **Validates: Requirements 2.1, 2.3**
   */
  it("Property 5.1: checkDependency('chromium') returns false and shows 'Setup Dependencies' when browserPath is undefined", async () => {
    await fc.assert(
      fc.asyncProperty(statusWithoutBrowserArb, async (status) => {
        ...
        // The gate SHALL return false
        expect(result).toBe(false);

        // An error message SHALL be shown containing "Setup Dependencies"
        expect(showErrorMessage).toHaveBeenCalledOnce();
        expect(showErrorMessage).toHaveBeenCalledWith(
          expect.stringContaining("Setup Dependencies")
        );
        // The message should also identify Chromium as the missing dependency
        expect(showErrorMessage).toHaveBeenCalledWith(
  ...
  /**
   * 5.2 Property: For any DependencyStatus where javaPath is undefined,
   * the java gate SHALL return false and the error message SHALL contain
   * "Setup Dependencies".
   *
   * **Validates: Requirements 2.1, 2.2**
   */
  it("Property 5.2: checkDependency('java') returns false and shows 'Setup Dependencies' when javaPath is undefined", async () => {
    await fc.assert(
      fc.asyncProperty(statusWithoutJavaArb, async (status) => {
        // Reset mocks for each property run
        showErrorMessage.mockClear();
        showWarningMessage.mockClear();
        ensureAllMock.mockClear();
        ...
        const result = ext.checkDependency("java");
        ...
        // The message should also identify Java/Corretto as the missing dependency
        expect(showErrorMessage).toHaveBeenCalledWith(
