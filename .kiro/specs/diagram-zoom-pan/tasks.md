...
- [ ] 1. Add diagram container wrapping in renderMarkdown.ts
  - [ ] 1.1 Wrap PlantUML SVG output in `<div class="diagram-container">...</div>` in `renderMarkdownDocument()`
  ...
  - [ ] 1.2 Wrap inline SVG output in `<div class="diagram-container">...</div>` in `renderMarkdownDocument()`
    - When an `svg` fenced block is processed, wrap the `sanitizeSvg()` output in a diagram-container div
    - _Requirements: 1.3, 1.4_
  - [ ] 1.3 Wrap Mermaid `.mermaid-host` placeholder in `<div class="diagram-container">...</div>` in `renderMarkdownDocument()`
    - When a Mermaid block renders successfully, wrap `result.placeholder` in a diagram-container div
    - _Requirements: 1.1, 1.4_
  - [ ]* 1.4 Write property test for diagram wrapping preserves content
  ...
  - [ ]* 1.5 Write unit tests for diagram container wrapping
    - Test that renderMarkdownDocument output contains `.diagram-container` divs around each diagram type
    - _Requirements: 1.1, 1.2, 1.3, 1.4_

- [ ] 2. Add diagram container and zoom indicator CSS styles in preview.css
  - [ ] 2.1 Add `.diagram-container` base styles
  ...
  - [ ] 2.2 Add `.zoom-indicator` overlay styles
  ...
  - [ ] 2.3 Add `@media print` rules for diagram containers
...
- [ ] 3. Checkpoint - Verify wrapping and styles
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Implement ZoomPanController in preview.js
  - [ ] 4.1 Add constants and `clamp` utility function
    ...
    - _Requirements: 2.2_
  - [ ] 4.2 Implement `applyTransform(container, state)` function
  ...
  - [ ] 4.3 Implement `handleWheel(event, container, state)` function
  ...
  - [ ] 4.4 Implement `handleMouseDown`, `handleMouseMove`, `handleMouseUp` functions
  ...
  - [ ] 4.5 Implement `handleDblClick(container, state)` function
  ...
  - [ ] 4.6 Implement `attachZoomPan(container)` and `initZoomPan()` functions
  ...
  - [ ]* 4.7 Write property test for zoom scale clamping
  ...
  - [ ]* 4.8 Write property test for cursor-centered zoom stability
  ...
  - [ ]* 4.9 Write property test for double-click reset
  ...
  - [ ]* 4.10 Write property test for pan displacement
  ...
  - [ ]* 4.13 Write property test for initialization idempotency
...
- [ ] 5. Checkpoint - Verify ZoomPanController logic
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Integrate ZoomPanController with existing preview lifecycle
  - [ ] 6.1 Call `initZoomPan()` after Mermaid rendering on DOMContentLoaded
    - In the `DOMContentLoaded` handler, call `initZoomPan()` after `renderMermaidBlocks()` resolves
    - _Requirements: 6.3_
  - [ ] 6.2 Call `initZoomPan()` after `update-body` message handler re-renders content
    - After `renderMermaidBlocks()` in the message handler, call `initZoomPan()`
    - _Requirements: 6.3_
  - [ ] 6.3 Re-initialize zoom/pan after theme-change re-render
  ...
  - [ ] 6.4 Prevent dblclick-to-jump conflict inside diagram containers
    ...
    - _Requirements: 4.1_
  - [ ] 6.5 Export new functions for testability
