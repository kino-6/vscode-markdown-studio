...
- [ ] 1. Add loading overlay CSS styles to preview.css
  - [ ] 1.1 Add `.ms-loading-overlay` base styles
...
- [ ] 2. Implement showLoadingOverlay and hideLoadingOverlay in preview.js
  - [ ] 2.1 Implement `showLoadingOverlay()` function
  ...
  - [ ] 2.2 Implement `hideLoadingOverlay()` function
  ...
  - [ ] 2.3 Export `showLoadingOverlay` and `hideLoadingOverlay` for testability
  ...
  - [ ] 2.5 Write unit tests for show/hide overlay
...
- [ ] 3. Update webview message handler in preview.js to handle render-start and render-error
  - [ ] 3.1 Add `render-start` message handling
  ...
  - [ ] 3.5 Write unit tests for message handler
...
- [ ] 4. Checkpoint - Verify webview-side changes
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Update webviewPanel.ts to send render-start and render-error messages
  - [ ] 5.1 Send `render-start` message before `renderBody()` in the change handler
  ...
  - [ ] 5.2 Send `render-error` message on `renderBody()` failure
  ...
  - [ ] 5.3 Apply same changes to the reuse path (when panel already exists)
  ...
  - [ ] 5.6 Write unit tests for render-start and render-error messages
...
- [ ] 6. Add initial load spinner support
  - [ ] 6.1 Include loading overlay in initial HTML from `buildHtml()`
    ...
    - _Requirements: 6.1_
  - [ ] 6.2 Hide spinner after initial Mermaid rendering in `DOMContentLoaded` handler
    - After `renderMermaidBlocks()` resolves in the `DOMContentLoaded` handler, call `hideLoadingOverlay()`
    - _Requirements: 6.2_
  - [ ] 6.3 Write unit test for initial load spinner
